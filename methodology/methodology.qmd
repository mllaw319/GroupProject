...
-   Correlation between average speed and: (i) reaction time and (ii) split times; using scatter plot (by gender, event, stroke, race distance and/or round). *\[Multivariate regression\]*

-   Test the hypothesis that mean average speed of swimmers for any particular event is not the same across teams or across continents. *\[Multivariate regression\]*
...
-   Patterns in distribution, to check for normality, using histogram (by gender, event, stroke, race distance and/or round). This may help provide the coach with benchmarks for performance.

-   Correlation between reaction time and final time, using scatter plot (by gender, event, stroke, race distance and/or round). *\[Multivariate regression\]*

-   Test the hypothesis that mean reaction times are not the same across the two genders, across events. *\[Multivariate regression\]*

-   Test the hypothesis that mean reaction times are not the same across race distances, keeping gender constant. *\[Multivariate regression\]* 
